...
export function MarginDistribution({
  ...
  return (
    <figure className="iei-fig iei-dist">
      <svg viewBox={`0 0 ${W} ${H + 7}`} role="img" aria-label={distLabel(bins, total)} preserveAspectRatio="none">
        {bins.map((b, i) => {
          const h = (b.n / peak) * H;
          ...
          return (
            <Link key={b.lo} href={hrefFor(on ? null : b.lo)}>
              <g opacity={selected === null || on ? 1 : 0.35}>
                ...
                <rect x={i * slot} y={0} width={slot} height={H + 7} fill="transparent" />
                <rect
                  x={i * slot + gap / 2}
                  y={H - h}
                  width={slot - gap}
        ...
        {/* Three ticks, not twenty-one. The axis is there to make the shape readable, not to be read. */}
        <g className="iei-dist-axis" aria-hidden="true">
          <text x={0} y={H + 5.5}>
            0%
          </text>
          <text x={W / 2} y={H + 5.5} textAnchor="middle">
            20%
          </text>
          <text x={W} y={H + 5.5} textAnchor="end">
            40%+
          </text>
        </g>
      </svg>
  ...
  return (
    <figure className="iei-fig iei-vs">
      <svg viewBox={`0 0 ${S} ${S}`} role="img" aria-label={vsLabel(shown)}>
        ...
        <text x={x(ceiling) - 1} y={y(ceiling) + 4} className="iei-vs-note" textAnchor="end">
          seats = votes
        </text>
        ...
        <g className="iei-vs-axis" aria-hidden="true">
          ...
          <text x={S - pad} y={S - pad + 6} textAnchor="end">
            vote share →
          </text>
          <text x={pad - 2} y={pad} transform={`rotate(-90 ${pad - 2} ${pad})`}>
            seat share →
          </text>
        </g>
          ...
          return (
            <Link key={r.party.key} href={hrefFor(on ? null : r.party.key)}>
              <g opacity={dim ? 0.3 : 1}>
                ...
                <circle cx={cx} cy={cy} r={on ? 3.4 : 2.6} fill={fillFor(r.party.key)} className="iei-vs-dot" />
                <text x={cx + 4} y={cy - 2.5} className="iei-vs-tag">
                  {r.party.label.length > 10 ? `${r.party.label.slice(0, 9)}…` : r.party.label}
                </text>
                <title>
